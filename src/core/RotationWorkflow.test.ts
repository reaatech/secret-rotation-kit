...
import { RotationWorkflow } from "./RotationWorkflow.js";
import { ProviderError, RotationError, VerificationError } from "./errors.js";
...
describe("RotationWorkflow", () => {
	...
	it("emits lifecycle events", async () => {
		...
		const emittedTypes = eventEmitter.emit.mock.calls.map((call) => call[0].type);
	...
	it("stores the generated key in the key store", async () => {
		...
		const savedKey = keyStore.save.mock.calls[0][0] as SecretKey;
	...
	it("cancels provider rotation and marks key failed on error", async () => {
		...
		const updatedKey = keyStore.update.mock.calls.find(
			(call) => (call[0] as SecretKey).status === "failed",
		);
