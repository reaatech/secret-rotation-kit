...
import { RotationManager, createRotationConfig } from "./RotationManager.js";
...
describe("RotationManager", () => {
	...
	it("createRotationConfig fills in defaults", () => {
		...
		expect(config.provider).toEqual({ type: "aws", region: "us-east-1" });
	...
	it("createRotationConfig preserves overrides", () => {
		const config = createRotationConfig({
			provider: { type: "gcp", projectId: "my-project" },
	...
	it("throws when no provider or providerInstance is provided", () => {
		expect(() => new RotationManager({})).toThrow(
			"Either provider or providerInstance must be provided",
		);
	});
	...
	it("uses custom rate limiter from config", () => {
		...
		const manager = new RotationManager({
			providerInstance: createMockProvider(),
			rateLimiter: customLimiter as Parameters<typeof RotationManager>[0]["rateLimiter"],
		});
