...
describe("ConfigService", () => {
	describe("create", () => {
		it("builds full config from minimal provider config", () => {
			const config = ConfigService.create({ type: "aws", region: "us-east-1" });
	...
	it("rejects __proto__ keys in overrides", () => {
		...
		const config = ConfigService.create(
			...
			overrides as Parameters<typeof ConfigService.create>[1],
		);
	...
	it("rejects constructor and prototype keys in overrides", () => {
		const config = ConfigService.create({ type: "aws", region: "us-east-1" }, {
			...
			prototype: "bad",
			verification: { timeout: 10000 },
		} as Parameters<typeof ConfigService.create>[1]);
	...
	describe("defaults", () => {
		it("returns a copy of defaults", () => {
			const d1 = ConfigService.defaults();
			const d2 = ConfigService.defaults();
			expect(d1).toEqual(d2);
			expect(d1).not.toBe(d2);
		});

		it("defaults include all sections", () => {
			const defaults = ConfigService.defaults();
