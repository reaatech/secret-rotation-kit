...
> and live alongside each package (e.g. `packages/core/CHANGELOG.md`). This file
> is a consolidated summary of the initial release; consult the per-package
> files for version-by-version detail going forward.
...
- **`secret-rotation-sidecar` CLI**: standalone, env-configured binary that boots a fully wired sidecar (provider selected via `SRK_PROVIDER`), with graceful shutdown. Also the default Docker image entry point.
