...
import { assertValid, validateSecretName } from './input-validator.js';
...
export class RotationManager {
  ...
  constructor(config: RotationManagerConfig) {
    ...
    if (config.providerInstance) {
      ...
      throw new Error('providerInstance must be provided');
    }
 ...
 *
 * @param config - Required providerInstance plus optional overrides.
 * @returns Ready-to-use RotationManagerConfig.
 */
export function createRotationConfig(
  config: { providerInstance: SecretProvider } & Partial<
    Omit<RotationManagerConfig, 'providerInstance'>
  >,
